...
import { signupUser, loginUser, logoutUser, onAuthChange, getProfile, listenProfile } from '@/services/api';
  ...
  persist(
    (set, get) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          const fbUser = await loginUser(parsed.data.email, parsed.data.password);
      ...
      signup: async (name: string, email: string, password: string, location: string) => {
        ...
        try {
          const fbUser = await signupUser(parsed.data.name, parsed.data.email, parsed.data.password, parsed.data.location);
          const profile = await getProfile(fbUser.uid);
        ...
        onAuthChange(async (firebaseUser) => {
          if (firebaseUser) {
            ...
            if (profile) {
              ...
              // Listen for real-time profile updates (balance changes, streak, etc.)
              listenProfile(firebaseUser.uid, (updatedProfile) => {
                set(state => ({
                  user: state.user ? {
                    ...state.user,
                    kruxBalance: updatedProfile.krux_balance,
                    greenScore: updatedProfile.green_score,
                    streak: updatedProfile.streak,
                    totalScans: updatedProfile.total_scans,
                    co2Saved: updatedProfile.co2_saved,
                    waterSaved: updatedProfile.water_saved,
                    plasticRecycled: updatedProfile.plastic_recycled,
